            ...
            {"label": _("Employee Name"), "fieldname": "employee_name", "fieldtype": "Data", "width": 120},
               {
                ...
                "fieldtype": "Float",
                "fetch_from": "employee.custom_salary_amount",
                "width": 135,
            },
               {
                "label": _("Salary"),
...
def get_data(filters: Filters, attendance_map: dict) -> list[dict]:
    ...
    if filters.group_by == "Project":
        ...
        for project in group_by_param_values:
            ...
            records = get_rows(emp_dict, filters, holiday_map, attendance_map, project)
            if records:
...
def get_rows(employee_details: dict, filters: Filters, holiday_map: dict, attendance_map: dict, project_name=None) -> list[dict]:
    ...
    for employee, details in employee_items:
        ...
        if filters.summarized_view:
            ...
            row = {"employee": employee, "employee_name": details.employee_name,"base_salary":details.custom_salary_amount,"salary": details.custom_salary_amount}
            ...
            # 	"salary": details.custom_salary_amount or 0
            # })
            # if filters.group_by == "Project":
            ...
            if filters.group_by == "Project":
                ...
                all_attendance_data = attendance_map.get(employee, {})
                total_present = 0
                for shift_status in all_attendance_data.values():
                    for day_status in shift_status.values():
                        if day_status == "Present" or day_status == "Work From Home":
                            total_present += 1
                        elif day_status == "Half Day":
                            total_present += 0.5
                records.extend(attendance_for_employee)
                full_salary = details.custom_salary_amount or 0
                prorated_salary = 0
                if total_present:
                    prorated_salary = round((present_days / total_present) * full_salary, 2)

                    attendance_for_employee[0].update({
                        "employee": employee,
                        "employee_name": details.employee_name,
                        "base_salary":details.custom_salary_amount,
